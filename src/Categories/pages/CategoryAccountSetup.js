...
import './CategoryAccountSetup.css'
import bankIcon from '../../assets/bankIcon.png'
import '../../pages/UserProfile.css'
import loadingImg from '../../assets/loading.gif'

const CategoryAccountSetup = () => {
    ...
    useEffect(() => {
        ...
        let getAccountInfo = async() =>{
            let response = await fetch(`${process.env.REACT_APP_API_URL}/partner-with-us/create-stripe-account/get-details/`, {
                method:'GET',
                headers:{
            ...
            // If category has not created any account yet (probably because landed for first time)
            if(response.status === 200){
                setAccountStatus('NotCreated')
            }
            // If category has created the account but has not provided all details to stripe
            else if(response.status === 230){
                setAccountStatus('NotCompleted')
            } 
            // If category has created the account and provided all the details (i.e. fully connected with stripe)
    ...
    let createStripeAccount = async() =>{
        setDisabled(true)
        let response = await fetch(`${process.env.REACT_APP_API_URL}/partner-with-us/create-stripe-account/`, {
            method:'POST',
            headers:{
                'Content-Type':'application/json',
                // Provide the authToken when making API request to backend to access the protected route of that user
    ...
    let completeStripeAccount = async() =>{
        setDisabled(true)
        let response = await fetch(`${process.env.REACT_APP_API_URL}/partner-with-us/complete-stripe-account/`, {
            method:'POST',
            headers:{
                'Content-Type':'application/json',
                // Provide the authToken when making API request to backend to access the protected route of that user
        ...
        <div>
            ...
            ?   <div> 
                    <div className='newAccount-setup-center'>
                        ...
                        <h2>Setup payouts to list on From Our Kitchen.</h2> 
                        From Our Kitchen partners with Stripe to transfer earnings to your (test) bank account. <br/>
                    ...
                    <div className='test-section'>
                        ...
                        <p className='test-details'> Industry: Software <br/> Website: https://github.com/Adityagupta0973 <br/> Desc: test stripe account for From Our Kitchen </p>
                        <p className='test-heading'> Section 5: Customer support details</p>
                        <p className='test-details'> Statement descriptor: [your category name] </p>
                    </div>
                </div> 
            : (null)}
...
export default CategoryAccountSetup
